...
test.describe('JF-509 ِApprove/Reject Facility Registration Request', () => {
  ...
  test.fixme('@fe JF-509 — UI: user completes the flow on screen', async ({ page }) => {
    // TODO(JF-509/FE): navigate the screen(s) and assert the ACs via getByRole/getByTestId.
    test.info().annotations.push({ type: 'story', description: 'JF-509' });
    test.info().annotations.push({ type: 'layer', description: 'fe' });
  });
  // BE (API): assert the endpoint contract / RBAC / data shape behind this story.
  test.fixme('@be JF-509 — API: endpoint contract & rules', async () => {
    // TODO(JF-509/BE): call the story's API (src/helpers/api.ts) and assert status + payload.
    test.info().annotations.push({ type: 'story', description: 'JF-509' });
    test.info().annotations.push({ type: 'layer', description: 'be' });
  });
  // DB: verify persisted state (SELECT-only, env-gated on CB_*).
  test.fixme('@db JF-509 — DB: persisted state matches', async () => {
    // TODO(JF-509/DB): guard with dbAvailable(); SELECT the affected row(s) and assert (src/db.ts).
    test.info().annotations.push({ type: 'story', description: 'JF-509' });
