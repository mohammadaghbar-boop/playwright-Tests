...
test.describe('JF-171 Inheritance Classsification/ تصنيف التركة', () => {
  ...
  test.fixme('@fe JF-171 — UI: user completes the flow on screen', async ({ page }) => {
    // TODO(JF-171/FE): navigate the screen(s) and assert the ACs via getByRole/getByTestId.
    test.info().annotations.push({ type: 'story', description: 'JF-171' });
    test.info().annotations.push({ type: 'layer', description: 'fe' });
  });
  // BE (API): assert the endpoint contract / RBAC / data shape behind this story.
  test.fixme('@be JF-171 — API: endpoint contract & rules', async () => {
    // TODO(JF-171/BE): call the story's API (src/helpers/api.ts) and assert status + payload.
    test.info().annotations.push({ type: 'story', description: 'JF-171' });
    test.info().annotations.push({ type: 'layer', description: 'be' });
  });
  // DB: verify persisted state (SELECT-only, env-gated on CB_*).
  test.fixme('@db JF-171 — DB: persisted state matches', async () => {
    // TODO(JF-171/DB): guard with dbAvailable(); SELECT the affected row(s) and assert (src/db.ts).
    test.info().annotations.push({ type: 'story', description: 'JF-171' });
