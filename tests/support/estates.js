...
function payload({
  nid = ARDHKHAWA_NID, heirCount = 2, re = [], deposits = [], vehicles = [],
  portfolios = [], movables = [], outside = false, court = COURT_CODE,
} = {}) {
  const s   = seq();
  ...
  return {
    ...
    deedInfo: {
      ...
      totalPages: 20,
      issuingAuthority: {
        country: 'المملكة العربية السعودية', ministry: 'وزارة العدل',
        court: 'محكمة الأحوال الشخصية بالرياض', department: 'الدائرة القضائية السابعة',
      },
    },
    deceased: {
      name: 'عبدالرحمن سالم فهد الدوسري', idType: 'هوية وطنية', idNumber: nid, nationality: 'SA',
      dateOfDeathHijri: '1446-01-20', estateDocumentNumber: `HC-1447-TC171-${uid}`,
      estateDocumentDate: '1447-05-10', estateDocumentIssuedBy: 'محكمة الأحوال الشخصية بالرياض',
      probateCertificateAttachment: `CERT-TC171-${uid}.pdf`,
    },
    caseInfo: {
      caseNumber: `1447/7/TC171-${uid}`, caseDate: '1447-06-15', caseType: 'تصفية تركة',
      status: CASE_STATUS, finalityMethod: FINALITY_METHOD, finalityStatement: 'صادقت محكمة الاستئناف على منطوق الحكم',
    },
    parties: {
    ...
    estateAssets: { realEstate: re, deposits, vehicles, investmentPortfolios: portfolios, movables },
    inheritanceDistribution: {
      legalBasis: 'المادة 180 من نظام الأحوال الشخصية', distributionRule: 'الأنصبة الشرعية', totalShares: heirCount,
      heirs: hs.map(h => ({
        name: h.name, idType: 'هوية وطنية', idNumber: h.idNumber, relation: h.relation,
        gender: h.relation === 'ابن' ? 'ذكر' : 'أنثى', sharesPerPerson: 1, totalShares: heirCount,
      })),
    },
    ruling: {
      verdictItems: [
    ...
    liquidationCenter: {
      name: 'مركز إنفاذ للتصفية', appointmentBasis: 'أمر قضائي', appointmentDate: '1448-01-25',
      feePercentage: 2.5, feeBase: 'إجمالي القيمة المتحصلة',
...
const deposits   = (n, v = 100000) => Array.from({ length: n }, (_, i) => deposit(v, i));
const vehicles   = (n, v = 50000)  => Array.from({ length: n }, (_, i) => vehicle(v, i));
const portfolios = (n, v = 500000) => Array.from({ length: n }, (_, i) => portfolio(v, i));
const movables   = (n, v = 10000)  => Array.from({ length: n }, (_, i) => movable(v, i));

/** POST a referral to create an estate. `portalPage` must be an authenticated portal page. */
async function createEstate(portalPage, p) {
...
function extractReferralNum(body) {
  return body?.internalReferralNumber || body?.referralNumber
      || body?.data?.internalReferralNumber || body?.data?.referralNumber
      || body?.internal_referral_number || null;
}

module.exports = {
  API_REFERRALS, COURT_API_KEY, TENANT_ID, ARDHKHAWA_NID, COURT_CODE, CASE_STATUS,
  payload, heirs, reAsset, deposit, vehicle, portfolio, movable, deposits, vehicles, portfolios, movables,
  createEstate, extractReferralNum,
};
