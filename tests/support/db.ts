...
/** Minimal shape of a CloudBeaver GraphQL response we read fields off. */
...
/** Authenticated CloudBeaver session exposing a typed async SQL runner. */
export interface DbHandle {
  page: Page;
  sql<T = Record<string, any>>(query: string): Promise<T[]>;
  close(): Promise<void>;
  readonly connId: string;
  readonly ctxId: string | null;
}

async function gql(page: Page, body: unknown, timeoutMs = 60000): Promise<GqlResponse> {
  ...
  try {
    return JSON.parse(txt) as GqlResponse;
  } catch {
    return { raw: txt };
  }
}
 ...
 */
export async function openDb(browser: Browser): Promise<DbHandle> {
  ...
  const pg = ((conns?.data?.userConnections as any[]) || []).find(
  ...
  const state: { connId: string; projectId: string; ctxId: string | null } = {
    connId: pg.id,
    projectId: pg.projectId,
    ctxId: null,
  };
  for (let attempt = 0; attempt < 6 && !state.ctxId; attempt++) {
    await gql(page, { query: `mutation { initConnection(id: "${state.connId}", projectId: "${state.projectId}") { connected } }` }).catch(() => undefined);
    ...
    state.ctxId = ctx?.data?.sqlContextCreate?.id ?? null;
  ...
  async function sql<T = Record<string, any>>(query: string): Promise<T[]> {
    ...
    if (!rs) return [];
    const cols: string[] = rs.columns.map((c: { name: string }) => c.name.replace(/^"|"$/g, ''));
    return rs.rows.map((row: unknown[]) => Object.fromEntries(cols.map((c, i) => [c, row[i]])) as T);
  }
