...
import { openDb, type DbHandle } from '../../support/db';
import {
  payload,
  reAsset,
  deposit,
...
import {
  ...
  breakdown,
  crit,
  assertCriterionConsistent,
  assertScoringConsistent,
  type CaseRow,
...
interface ScenarioResult {
  ...
  refNums: (string | null)[];
  caseIds: (string | null)[];
  caseRows: (CaseRow | null)[];
}

let portalPage: Page;
let db: DbHandle;
const RESULTS: Record<string, ScenarioResult> = {}; // key -> per-estate arrays
...
function res(key: string, idx = 0): { createStatus: number | undefined; caseId: string | null; caseRow: CaseRow | null } {
  const r = RESULTS[key] || { createStatus: [], refNums: [], caseIds: [], caseRows: [] };
  return { createStatus: r.createStatus[idx], caseId: r.caseIds[idx] ?? null, caseRow: r.caseRows[idx] ?? null };
}
function requireClassified(label: string, caseId: string | null, caseRow: CaseRow | null): void {
...
test.describe('JF-171 — Estate Classification', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const flat: Array<{ key: string; idx: number }> = [];
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
      ...
      try {
        rows = await db.sql(`SELECT internal_referral_number, court_case_id FROM cases.referral_requests WHERE internal_referral_number IN (${refList})`);
      } catch {
      ...
      for (const f of pending) {
        const rn = RESULTS[f.key].refNums[f.idx];
        if (rn && map[rn]) RESULTS[f.key].caseIds[f.idx] = map[rn];
      }
    ...
    while (Date.now() < deadline) {
      ...
      let rows: CaseRow[] = [];
      try {
        rows = await db.sql<CaseRow>(`SELECT id, classification, classification_result, total_classification_score, criterion_breakdown_json, rank FROM cases.court_cases WHERE id IN (${idList})`);
      } catch {
        await db.page.waitForTimeout(15000);
        continue;
      }
      const byId: Record<string, CaseRow> = {};
      for (const r of rows) byId[String(r.id)] = r;
      for (const f of pending) {
        const row = byId[RESULTS[f.key].caseIds[f.idx] as string];
  ...
  test('TC-171-06 — Only Not Ready assets are included in classification scope', async () => {
    ...
    const assets = await getAssets(db, caseId as string);
    ...
    const c = crit(caseRow as CaseRow, 'classifiedAssetsCount');
  ...
  function guardNotApplicable(label: string, assets: Array<Record<string, unknown>>): void {
    const notReady = assets.filter((a) => String(a.status) === '10');
  ...
  test('TC-171-09 — Classification result is Not Applicable when all assets are Prelim Ready', async () => {
    ...
    guardNotApplicable('TC-09', await getAssets(db, caseId as string));
    const classif = String((caseRow as CaseRow).classification ?? '').toLowerCase();
    expect(classif.includes('not_applicable') || classif.includes('لا ينطبق') || classif === 'na', `Expected Not Applicable, got: ${(caseRow as CaseRow).classification}`).toBeTruthy();
    console.log(`TC-09 PASS | caseId=${caseId} | classification=${(caseRow as CaseRow).classification}`);
  });
  ...
  test('TC-171-10 — No rank is saved when result is Not Applicable', async () => {
    ...
    guardNotApplicable('TC-10', await getAssets(db, caseId as string));
    const rank = (caseRow as CaseRow).rank;
    expect(rank === null || rank === undefined || String(rank) === '').toBeTruthy();
    console.log(`TC-10 PASS | caseId=${caseId} | rank=${rank}`);
  });
  ...
  test('TC-171-11 — Liquidator assignment not triggered when result is Not Applicable', async () => {
    ...
    guardNotApplicable('TC-11', await getAssets(db, caseId as string));
    const events = await getCaseEvents(db, caseId as string);
  ...
  test('TC-171-62 — Audit log shows result Not Applicable', async () => {
    ...
    guardNotApplicable('TC-62', await getAssets(db, caseId as string));
    const events = await getCaseEvents(db, caseId as string);
    ...
    const cls = (caseRow as CaseRow).classification;
    const hasNA = detail.includes('لا ينطبق') || detail.toLowerCase().includes('not_applicable') || detail.toLowerCase().includes('na') || (cls != null && String(cls).toLowerCase().includes('not_applicable'));
  ...
  test('TC-171-70 — Liquidator assignment not triggered (Not Applicable) — table check', async () => {
    ...
    guardNotApplicable('TC-70', await getAssets(db, caseId as string));
    const events = await getCaseEvents(db, caseId as string);
    ...
    const rows = await db.sql<{ cnt: unknown }>(`SELECT COUNT(*) AS cnt FROM cases.court_cases WHERE id = '${caseId}' AND liquidator_id IS NOT NULL`).catch(() => [{ cnt: 'na' }]);
  ...
  for (const key of ['TC-12', 'TC-13', 'TC-14']) {
    test(`TC-171-${key.slice(3)} — classifiedAssetsCount maps raw value → correct band`, async () => {
      ...
      const c = assertCriterionConsistent(caseRow as CaseRow, 'classifiedAssetsCount');
  ...
  test('TC-171-17 — classifiedAssetsCount band mapping consistent across two estates', async () => {
    ...
    const a = assertCriterionConsistent(A.caseRow as CaseRow, 'classifiedAssetsCount');
    const b = assertCriterionConsistent(B.caseRow as CaseRow, 'classifiedAssetsCount');
  ...
  for (const key of ['TC-26', 'TC-27', 'TC-28', 'TC-29']) {
    test(`TC-171-${key.slice(3)} — estimatedValueImpact maps raw value → correct band`, async () => {
      ...
      expect(createStatus).toBeLessThan(300);
      requireClassified(key, caseId, caseRow);
      const c = assertCriterionConsistent(caseRow as CaseRow, 'estimatedValueImpact');
  ...
  test('TC-171-30 — Movables have a criterion separate from value impact', async () => {
    ...
    expect(crit(caseRow as CaseRow, 'movablesCount'), 'movablesCount criterion must exist').toBeTruthy();
    expect(crit(caseRow as CaseRow, 'estimatedValueImpact'), 'estimatedValueImpact criterion must exist').toBeTruthy();
    assertCriterionConsistent(caseRow as CaseRow, 'estimatedValueImpact');
  ...
  for (const key of ['TC-31', 'TC-32', 'TC-35']) {
    test(`TC-171-${key.slice(3)} — movablesCount maps raw value → correct band`, async () => {
      ...
      expect(createStatus).toBeLessThan(300);
      requireClassified(key, caseId, caseRow);
      const c = assertCriterionConsistent(caseRow as CaseRow, 'movablesCount');
  ...
  for (const key of ['TC-38', 'TC-39', 'TC-40']) {
    test(`TC-171-${key.slice(3)} — heirsCount maps raw value → correct band`, async () => {
      ...
      expect(createStatus).toBeLessThan(300);
      requireClassified(key, caseId, caseRow);
      const c = assertCriterionConsistent(caseRow as CaseRow, 'heirsCount');
  ...
  test('TC-171-46 — Scoring math + rank mapping consistent; scope Yes→100', async () => {
    ...
    expect(assertCriterionConsistent(caseRow as CaseRow, 'estateOutsideKingdom').normalizedScore, 'outside=true → 100').toBe(100);
    const { total, rank } = assertScoringConsistent('TC-46', caseRow as CaseRow);
  ...
  test('TC-171-47 — Scoring math + rank mapping consistent; scope No→0', async () => {
    ...
    expect(assertCriterionConsistent(caseRow as CaseRow, 'estateOutsideKingdom').normalizedScore, 'outside=false → 0').toBe(0);
    const { total, rank } = assertScoringConsistent('TC-47', caseRow as CaseRow);
  ...
  for (const [id, why] of Object.entries(IMPOSSIBLE)) {
    test(`${id} SKIP — mathematically impossible with 5-criterion model`, async () => {
      test.skip(true, `${id} BLOCKED: ${why} Pending business decision (see JF-171 comment).`);
    });
  }

  const FAULT: Record<string, string> = { 'TC-171-56': 'calc failure', 'TC-171-61': 'all-retry failure', 'TC-171-64': '1st-attempt failure', 'TC-171-65': 'two failures', 'TC-171-66': 'three failures', 'TC-171-67': 'save-step failure', 'TC-171-68': 'persistent failure' };
  for (const [id, what] of Object.entries(FAULT)) {
    test(`${id} SKIP — requires fault injection (${what})`, async () => {
      test.skip(true, `${id} BLOCKED: requires technical-failure injection — agreed Not Doable (see JF-171 comment).`);
    });
  }
